...
#
# 可覆盖环境变量：
#   VLESS_SOURCE      binary（默认，下载预编译产物）/ source（现场编译）
#   VLESS_ENABLE_BBR  1（默认，启用 TCP BBR）/ 0（跳过）
#   VLESS_PORT        明文端口（默认 1080）
#   VLESS_TLS_PORT    TLS 端口（默认 8848，自签证书保底）
#   VLESS_LOG_LEVEL   日志等级（默认 info）
#   VLESS_USE_DOCKER  auto / 1 / 0（默认 auto：有可用 Docker 则用容器启动）
#   VLESS_PUBLIC_HOST 公网地址（域名/IP）；生成配置文件 host 字段，
#                     部署后服务端日志输出 vless:// 分享链接与二维码
#   VLESS_WORKDIR     源码 clone 目录（默认 /opt/cppvless，仅 source 模式用）
# =============================================================================
set -euo pipefail
...
WORKDIR="${VLESS_WORKDIR:-/opt/cppvless}"
BIN_DIR="/usr/local/bin"
CONFIG_DIR="/etc/vless"
...
LOG_LEVEL="${VLESS_LOG_LEVEL:-info}"
SOURCE_MODE="${VLESS_SOURCE:-binary}"
ENABLE_BBR="${VLESS_ENABLE_BBR:-1}"
...
enable_bbr() {
  [ "${ENABLE_BBR}" != "1" ] && { info "跳过 BBR 优化（VLESS_ENABLE_BBR=0）"; return; }
...
case "${SOURCE_MODE}" in
  binary)
    ...
    case "$(uname -m)" in
      ...
      *)
        err "不支持的架构 $(uname -m)：仅发布 amd64/arm64 预编译产物，请改用 VLESS_SOURCE=source"
        exit 1
        ;;
    esac
    ...
    curl -fsSL -o "${TMP_DL}/${TARBALL}" "${RELEASE_BASE}/${TARBALL}" \
      || { err "下载失败，请确认仓库已发布 v* tag 的 Release；或改用 VLESS_SOURCE=source"; exit 1; }
    ...
    SERVER_BIN="${TMP_DL}/vless_server"
    CLIENT_BIN="${TMP_DL}/vless_client"
    info "预编译产物校验通过"
    ;;

  source)
    # 现场编译
    if [ -f CMakeLists.txt ] && grep -q "project(vless" CMakeLists.txt 2>/dev/null; then
      SRC_DIR="$(pwd)"
    ...
    if [ "${USE_DOCKER}" -eq 1 ]; then
      ...
      bash ./build.sh
      SERVER_BIN="build-docker/src/vless_server"
      CLIENT_BIN="build-docker/src/vless_client"
    else
      ...
      SERVER_BIN="build/src/vless_server"
      CLIENT_BIN="build/src/vless_client"
    fi
    ;;
  *)
    err "未知 VLESS_SOURCE=${SOURCE_MODE}（可选 binary / source）"
    exit 1
    ;;
esac

# ── 3. 安装二进制 ──────────────────────────────────────────────────────────
info "安装 vless_server → ${BIN_DIR}/"
install -m 0755 "${SERVER_BIN}" "${BIN_DIR}/vless_server"
if [ -f "${CLIENT_BIN}" ]; then
  install -m 0755 "${CLIENT_BIN}" "${BIN_DIR}/vless_client"
  info "安装 vless_client → ${BIN_DIR}/"
fi
...
EOF
  if [ -n "${VLESS_PUBLIC_HOST:-}" ]; then
    info "已设置公网地址 host=${VLESS_PUBLIC_HOST}，启动日志将输出分享链接与二维码"
  fi
else
...
if [ "${USE_DOCKER}" -eq 1 ]; then
  ...
  cp "${BIN_DIR}/vless_server" "${TMP_CTX}/"
...
[Service]
ExecStart=${BIN_DIR}/vless_server --config ${CONFIG_FILE} --log-file ${LOG_FILE}
Restart=on-failure
RestartSec=3
LimitNOFILE=1048576
...
EOF
  ...
  info "以 nohup 后台启动"
  pkill -x vless_server 2>/dev/null || true
  sleep 1
  setsid "${BIN_DIR}/vless_server" --config "${CONFIG_FILE}" --log-file "${LOG_FILE}" \
    >/tmp/vless.out 2>&1 </dev/null &
fi
...
echo
echo "  ${BIN_DIR}/vless_client --remote ${PUBLIC_HOST:-SERVER_IP}:${TLS_PORT} --uuid ${UUID} --socks5-port 1080"
echo
info "══════════════════════════════════"
